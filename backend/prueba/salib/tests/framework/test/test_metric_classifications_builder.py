...
class TestMetricClassificationsBuilder(unittest.TestCase):
    ...
    def test_no_overlap(self):
        anomalies = [
            ...
            anomaly.Anomaly.from_epoch(2, 6, 1.0)
        ]
    ...
    def test_overlap(self):
        anomalies = [
            anomaly.Anomaly.from_epoch(1, 4, 1.0),
            anomaly.Anomaly.from_epoch(3, 6, 1.0)
        ]
        anomalies = sorted(anomalies)
    ...
    def test_deep_overlap(self):
        anomalies = [
            anomaly.Anomaly.from_epoch(1, 2, 1.0),
            anomaly.Anomaly.from_epoch(3, 4, 1.0),
            anomaly.Anomaly.from_epoch(1, 5, 1.0)
        ]
        anomalies = sorted(anomalies)
        self.assertRaises(Exception, cb.assert_no_overlap, anomalies)
